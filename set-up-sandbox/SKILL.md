...
- The template generates no `allowedDomains`/`deniedDomains`: on 0.6.5 the domain keys of a project-local `.pi/sandbox.json` are silently ignored — pi-sandbox's built-in default allowlist governs (`registry.npmjs.org` 200, other domains blocked even when explicitly allowed), so writing them would be config that looks like a boundary and isn't. Do not promise open egress; use probe 4 to discover which domains the project needs actually resolve and tell the user. If an upgrade starts honouring them, adopt them then — and never with a `"*"` in `deniedDomains`, which hard-blocks every request regardless of `allowedDomains`.
