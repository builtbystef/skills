...
The recommended place for a loop is a repository that carries the native sandbox config from the `set-up-sandbox` skill (`.claude/settings.json` for Claude Code, `.pi/sandbox.json` plus `.pi/extensions/pi-permission-system/config.json` for Pi). The iterations then run without prompts, writes stay confined to the repository, and the protected paths stay unreadable. If none of that config exists, suggest invoking the `set-up-sandbox` skill first, then this skill again. A run without a sandbox is the user's decision. Do not block on it. But say plainly that each iteration then has full, unconfined access to the machine. Codex is never a loop runner: it has no read confinement in any mode, so an unattended Codex session can read the protected directories — see [references/runners.md](references/runners.md).
