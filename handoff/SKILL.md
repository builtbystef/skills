...
2. **Give the handoff to the user.** Show the full path. Show the first command for the next session, for example `Read /tmp/handoffs/{{repo}}/{{file}}, then /implement`. Tell the user that the system clears the temp folder at reboot. If a handoff must stay available longer, put it in a tracker note on the applicable issue. This step is complete when the user has the path and the first command.
